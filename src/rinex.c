    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        //首先进行第一行版本号读取，记录版本号以及观测文件类型
            ...
            *type=*(buff+20);
            ...
            /* satellite system */  //通过定位字符位置读取字符，判断系统类型并记录
            switch (*(buff+40)) {
                case ' ':
        ...
        switch (*type) { /* file type */    //通过判断文件类型分配不同函数读取文件头
                       ...
                       sta_t *sta)
{
    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        /* decode observation epoch */
        if (i==0) { //如果是第一行，则调用decode_obsepoch()函数解码首行数据（包括历元时刻、卫星数、卫星编号、历元状态等信息），并将信息保存
        ...
        //如果不是第一行则调用decode_obsdata()函数对该行观测数据进行数据解码
            ...
            /* decode RINEX observation data */ //记录历元的数据部分
                     ...
                     obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    /* read RINEX file header */    //读取rinex头文件,获取文件类型type
    if (!readrnxh(fp,&ver,type,&sys,&tsys,tobs,nav,sta)) return 0;
    
    /* flag=0:except for clock,1:clock */   //type:通过readrnxh()读取文件头第一行获得
                       ...
                       obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    /* read RINEX file */   //调用readrnxfp，从文件描述符fp中读取文件
                    ...
                    sta_t *sta)
{
    ...
    if (type=='O'&&sta) {   //如果测站名字为空，就给依据头文件自动赋4个字符的名字
