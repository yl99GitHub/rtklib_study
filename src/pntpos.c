                   ...
                   double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        /* time system offset and receiver bias correction */       // 处理不同系统（GPS、GLO、GAL、CMP）之间的时间偏差，修改矩阵 H
        ...
        /* variance of pseudorange error */     // 调用 varerr 函数，计算此时的导航系统误差，然后累加计算用户测距误差(URE)。
        var[nv++]=varerr(opt,azel[1+i*2],sys)+vare[i]+vmeas+vion+vtrp;
        
        trace(4,"sat=%2d azel=%5.1f %4.1f res=%7.3f sig=%5.3f\n",obs[i].sat,
              azel[i*2]*R2D,azel[1+i*2]*R2D,resp[i],sqrt(var[nv-1]));
    }
    /* constraint to avoid rank-deficient */        //为了防止不满秩的情况，把矩阵H补满秩了
    for (i=0;i<NX-3;i++) {
        if (mask[i]) continue;
        v[nv]=0.0;
                  ...
                  double *resp, char *msg)
{
    ...
    for (i=0;i<MAXITR;i++) {    //开始迭代定位计算,迭代次数MAXITR默认为10
        ...
        if ((info=lsq(H,v,NX,nv,dx,Q))) {   //调用 lsq 函数,得到当前 x 的修改量 dx 和定位误差协方差矩阵中的权系数阵 Q
            sprintf(msg,"lsq error info=%d",info);
            break;
        }
        for (j=0;j<NX;j++) {
            x[j]+=dx[j];        //将lsq中求得的 dx 加入到当前 x 值中，得到更新之后的 x 值
        }
        if (norm(dx,NX)<1E-4) { //这个条件 norm(dx,NX)<1E-4 表示向量 dx 的2范数（Euclidean norm，即向量元素平方和的平方根）小于1E-4。
            sol->type=0;
